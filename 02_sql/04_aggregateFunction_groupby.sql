...
select count(distinct ifnull(dept_name, 'a')) from emp; ## null인 것도 포함해서 count할 때

--  커미션 비율(comm_pct)이 있는 직원의 수를 조회
select count(comm_pct) from emp;

--  최고 급여액과 최저 급여액 그리고 그 둘의 차액을 출력
select min(salary), mqx(salary), max(salary) - min(salary)
...
-- comm_pct 평균
select avg(comm_pct), -- comm_pct가 있는 35명 기준 평균(null 을 제외하고 평균 계산)
	   avg(ifnull(comm_pct, 0)) -- 전체 직원에 대한 평균(null -> 0 변경할 뒤 평균을 계산)
from emp ;
