...
# 처음 시작할 때 한번 실행하고 반환되는 리소스를 메모리에 저장해놓고 다음부터는 그것을 사용
def get_model():
...
# 어떤 값들을 계속 유지해야 할 때 저장하는 공간(dict타입): session_state
...
st.text_input(
    ...
    key = "input_text" # session_state에 지정한 key로 등록되고 값은 입력폼의 value가 저장되어 동기화 된다.
                       # session_state값을 변경하면 입력 폼의 값이 변경, 반대도 마찬가지
)
